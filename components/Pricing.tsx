    ...
    <section id="pricing" className="mt-16">
      <Container>
        <div className="flex items-center justify-between">
          ...
          <span className="text-sm text-slate-600">{i.pricing.monthly}</span>
        </div>
        <div className="mt-6 grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {tiers.map((tier, idx) => (
            <div key={tier.name} className={"card p-6 " + (idx === 1 ? "ring-2 ring-brand-500" : "")}>
              {idx === 1 && <p className="text-xs uppercase tracking-widest text-brand-700 mb-2">{i.pricing.mostPopular}</p>}
